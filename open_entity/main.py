...
args.entity_vocab = EntityVocab(args.data_dir + 'entity_vocab.tsv')
args.tokenizer = RobertaTokenizer('data/vocab.json', 'data/merges.txt')
...
def run(args):
    ...
    if args.do_train:
        ...
        #state_dict = paddle.load('/home/aistudio/data/data124661/paddle_luke_op.pt')
        #state_dict = change_state_dict(state_dict, args.model_config)
        #args.model_weights['typing.weight'] = state_dict['typing.weight']
        #args.model_weights['typing.bias'] = state_dict['typing.bias']
...
def evaluate(args, model, fold="dev", output_file=None):
    ...
    for logits, labels in zip(all_logits, all_labels):
        all_predicted_indexes.append([i for i, v in enumerate(logits) if v > -0.15])
