...
parser.add_argument("--data_dir", default="squad_data/squad_change/", type=str)
...
parser.add_argument("--pretrain_model", default='paddle_luke.pt')
parser.add_argument("--weight_decay", type=float, default=0.01)
parser.add_argument("--warmup_proportion", type=float, default=0.06)
parser.add_argument("--learning_rate", type=float, default=15e-6)
parser.add_argument("--adam_b1", type=float, default=0.9)
parser.add_argument("--adam_b2", type=float, default=0.98)
parser.add_argument('--tokenizer', default=RobertaTokenizer('squad_data/vocab.json', 'squad_data/merges.txt'))
...
def run(args):
    ...
    if args.do_train:
        paddle.save(model.state_dict(), os.path.join(args.output_dir, 'luke.pt'))

    if args.do_eval:
        args.do_train = False
        model = LukeForReadingComprehension(args)

        model.set_state_dict(paddle.load(os.path.join(args.output_dir, 'luke.pt')))
...
def evaluate(args, model, prefix=""):
    ...
    args.data_dir = 'squad_data/squad/'
